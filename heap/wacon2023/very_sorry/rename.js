...
(function (f) {
    ...
    */
    ...
    var bn = w(bt),
        ...
        input = readinput(bn, 0, fP);
    if (0 === arraylen(input) % 3) {
        var bl = 0,
            index = 0;
        for (;;) {
            if (arraylen(input) < index + 3) var bm = 0;
            else
                var fv = index(input, index),
                    fw = index(input, index + 1),
                    bm = [0, [0, fv, fw, index(input, index + 2)]];
            if (bm) {
                var instr = [0, bm[1], bl],
                    index = index + 3;
                continue;
            ...
            for (;;) {
                if (aU) {
                    var instrlist = [0, instr[1], instrlist],
                        instr = instr[2],
                    continue;
                }
                instrlist = [0, instrlist];
                break;
            }
            break;
        }
    } else var instrlist = 0;
    if (instrlist) {
        instrlist = instrlist[1];
        for (;;) {
            if (instrlist) {
                var curr = instrlist[1],
                    next = instrlist[2];
                aE(fx);
                // var fy = a[1];
                // all the Q(...) are print statements
                // this is probaly the opcode logging part
                // Q(aE(fz), fy);
                // var fA = a[2];
                // Q(aE(fB), fA);
                // var fC = a[3];
                ...
                (get|set)_memory
                syscall
                    Are defined in libstorage.so
                */
                if (17 < opcode) var u = -1;
                else
                    switch (opcode) {
                    case 0:
                        var u = set_register(curr[2], curr[3]);
                        break;
                    case 1:
                        var u = get_register(curr[2]);
                        break;
                    case 2:
                        var fF = get_register(curr[2]),
                            u = fF + get_register(curr[3]);
                        break;
                    case 3:
                        var fG = get_register(curr[2]),
                            u = set_register(0, fG + get_register(curr[3]));
                        break;
                    case 4:
                        var fH = get_register(curr[2]),
                            u = fH - get_register(curr[3]);
                        break;
                    case 5:
                        var fI = get_register(curr[2]),
                            u = set_register(0, fI - get_register(curr[3]));
                        break;
                    case 6:
                        var fJ = get_register(curr[2]),
                            u = Math.imul(fJ, get_register(curr[3]));
                        break;
                    case 7:
                        var fK = get_register(curr[2]),
                            u = set_register(0, Math.imul(fK, get_register(curr[3])));
                        break;
                    case 8:
                        var u = get_memory(get_register(curr[2]));
                        break;
                    case 9:
                        var u = get_memory(get_register[2]);
                        break;
                    case 10:
                        var u = set_register(0, get_memory(get_register(curr[2])));
                        break;
                    case 11:
                        var u = set_register(0, get_memory(curr[2]));
                        break;
                    case 12:
                        var u = set_memory(curr[2], curr[3]);
                        break;
                    case 13:
                        var fL = get_register(curr[3]),
                            u = set_memory(curr[2], fL);
                        break;
                    case 14:
                        var fM = get_register(curr[2]),
                            u = set_memory(fM, get_register(curr[3]));
                        break;
                    case 15:
                        var u = syscall(0);
                        break;
                    case 16:
                        var fN = get_register(curr[3]),
                            u = set_register(curr[2], fN);
                        break;
                    default:
                        var fO = get_register(curr[2]),
                            u = set_memory(fO, curr[3]);
                    }
                // probably printing the result value
                // Q(aE(fE), u);
                var instrlist = next;
                continue;
            }
