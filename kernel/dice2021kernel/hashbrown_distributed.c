...
// diff to normal add: new entry will be first item in list
static noinline void resize_add(uint32_t idx, hash_entry *entry, hash_entry **new_buckets)
{
    if (!new_buckets[idx])
    {
...
static long add_key(uint32_t idx, uint32_t key, uint32_t size, char *src)
{
    ...
    // > 1 item in bucket idx. If same key do nothing. Else add to bucket
    else
    {
        ...
        // ? why check again here, ah due to for loop structure
        if (temp->key == key)
        {
            kfree(temp_data);
            kfree(temp_entry);
        ...
        // insert new item to the end of bucket
...
static long delete_key(uint32_t idx, uint32_t key)
{
    ...
    // first in bucket free it, make next the first item in bucket
    ...
    // not found immediately. Loop through bucket
    ...
    // close hole so we can delete entry
    prev->next = temp->next;
    if (temp->value)
    {
        kfree(temp->value);
...
static long update_value(uint32_t idx, uint32_t key, uint32_t size, char *src)
{
    ...
    // search for value to update
    for (temp = hashmap.buckets[idx]; temp != NULL; temp = temp->next)
    {
        if (temp->key == key)
        {
            // if updated value is different, gotta free and alloc
            if (temp->size != size)
            {
                if (temp->value)
                {
            ...
            {
                // size is not different so simply update
